;+
; NAME: COLBAR.pro
;       (Lifted from D. Fanning's Colorbar.pro)
;
;
; $Id$
;
...
;
; SIDE EFFECTS:
;       Color bar is drawn to the current graphics device. The drawing
;       of the bounding box using 'plot' destroys whatever plotting
;       transformation existed before the call to colbar. If this
;       transformation is required after the call to colbar.pro, it
...
PRO COLBAR, BOTTOM=bottom, $  ; bottom color index
...
IF postScriptDevice THEN BEGIN
   ...
   IF true THEN BEGIN 
     ...
     bar = [ [[r[bar]]],[[g[bar]]],[[b[bar]]] ]
     TV, bar, xstart, ystart, XSIZE=xsize, YSIZE=ysize, true=3
   ENDIF  ELSE TV, bar, xstart, ystart, XSIZE=xsize, YSIZE=ysize
...
ENDIF ELSE BEGIN
   ...
   IF true THEN BEGIN    
     ...
     ENDIF ELSE tvlct,r,g,b,/get 
     bar =  [ [[r[bar]]], [[g[bar]]], [[b[bar]]] ]
     TV,bar,xstart,ystart,true=3
   ENDIF ELSE TV, bar, xstart, ystart
