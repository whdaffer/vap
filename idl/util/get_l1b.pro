FUNCTION get_l1b, file, limits
  
  catch, error
  IF error NE 0 THEN BEGIN 
    catch,/cancel
    IF n_elements(sd) THEN BEGIN hdf_sd_end,sd
    message,!error_state.msg,/cont
    return,0
  ENDIF 

  IF n_params() LT 2 THEN BEGIN 
    Message,"Usage: data=get_l1b(file,limits)",/cont
    return,0
  ENDIF 
  ...
  IF sd GE 0 THEN BEGIN 
    ...
    IF nx NE 0 THEN BEGIN 
      ...
      r = hdf_sd_nametoindex(sd,'cell_sigma0')
      sdsid = hdf_sd_select(sd,r)
      ...
      IF nx LT n_elements(sigma0) THEN BEGIN 
        ...
        mode = mode[x]
      ENDIF 
      junk =  {lon: 0., lat:0., sig0: 0., mode:0}
      data =  replicate(junk,n_elements(junk))
