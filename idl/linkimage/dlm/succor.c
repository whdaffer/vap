...
static IDL_VPTR IDL_CDECL succor( int argc, IDL_VPTR argv[], char * argk )
/* succor.c - IDL DLM
 * 
 *
 * call: IDL> succor, lon,lat,u,v,ui,vi,lonpar,latpar,ermax,rainf
 *
...

{
...
#ifdef DEBUG
  printf("succor: checking number of args\n");
#endif 
  if ( argc < 10 ) {
    IDL_Message( IDL_M_GENERIC, 
		 IDL_MSG_LONGJMP | IDL_MSG_ATTR_SYS,
		 "usage: succor, lon, lat, u, v, ui, vi, lonpar, latpar, ermax, rainf" );
  }

#ifdef DEBUG
  printf("succor: pulling out arguments\n");
#endif 
  ...
  for (i=0;i<=10;i++) 
    printf("(latd,lond,ud,vi)+%d = (%f,%f,%f,%f)\n",i,*(latd+i),*(lond+i),*(ud+i),*(vd+i));
  ...
  printf("succor: latmin, latmax, latinc = %f,%f,%f\n", latmin, latmax, latinc );
#endif
...
#ifdef DEBUG 
  printf(" succor: l,m,l1,m1,nvect = %ld,%ld,%ld,%ld,%ld\n",
	 l,m,l1,m1,nvect );
#endif
 ...
 printf(" getting memory for rinfo \n");
  ...
  if ((rinfo = (float*) 
    ...
    */
    ...
    printf("succor: about to call objanl2\n");
    ...
    printf("succor: done with objanl2\n");
  } else {
    IDL_Message( IDL_M_GENERIC, IDL_MSG_LONGJMP, 
...
int IDL_Load(void)
{
...
#ifdef DEBUG
  printf("In IDL_Load: about to call IDL_MessageDefineBlock\n");
#endif 

  if (! (msg_block = IDL_MessageDefineBlock("SUCCOR",
					    IDL_CARRAY_ELTS(msg_arr),
					    /*ARRLEN(msg_arr),*/
					    msg_arr))){
#ifdef DEBUG
    printf("IDL_Load: IDL_MessageDefineBlock failed: return IDL_FALSE\n");
#endif
    return IDL_FALSE;
  }
  ...
  if (!succor_startup()){
    IDL_MessageFromBlock(msg_block,SUCCOR_ERROR,
			 IDL_MSG_LONGJMP,
			 "Unable to initialize succor_with_data");
#ifdef DEBUG
    printf("IDL_Load: succor_startup failed: returning IDL_FALSE\n");
#endif
    return IDL_FALSE;
  }
...
int succor_startup(void)
{
#ifdef DEBUG
  printf("In succor_startup: about to call IDL_SysRtnAdd\n");
#endif
  static IDL_SYSFUN_DEF2 succor_procedures[] = {
    { (IDL_SYSRTN_GENERIC) succor,"SUCCOR",10,10,0,0},
  };


  if (!IDL_SysRtnAdd(succor_procedures,IDL_FALSE, 
                     ARRLEN(succor_procedures))){
#ifdef DEBUG
    printf("IDL_SysRtnAdd failed: about to return IDL_FALSE\n");
#endif     
    IDL_MESSAGE(IDL_M_GENERIC,IDL_MSG_RET,"Error adding succor_procedures\n");
    return(IDL_FALSE);
  }

#ifdef DEBUG
  printf("succor_startup: After IDL_SysRtnAdd\n");
#endif 
  /* Exit handler */
  IDL_ExitRegister(succor_exit);
#ifdef DEBUG
  printf("succor_startup: after IDL_ExitRegister, returning IDL_TRUE\n");
#endif 
  return(IDL_TRUE);
}
...
void succor_exit(void)
{
  /* nothing to do */
#ifdef DEBUG
  printf("In succor_exit\n");
#endif
}
