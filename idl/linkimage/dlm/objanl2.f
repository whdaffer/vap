      SUBROUTINE objanl2( sp,rinfo,ug,vg,l,m,nvect,nreps,time,
     1 sumx,sumy,a,store,rltmn,rlnmn,rltinc,rlninc,nvesc,trainf,
     2 gamma,denom,ermx,rainf,udev,vdev,alat,work, maxl, maxm)
c ********************************************************************
C
...
c
c Revision 1.2  1999/10/21 22:33:28  vapuser
c uncommented calls to 'smooth'
c
...
C ---------- Arguments -----------------
      ...
      integer*4 l,m,maxl, maxm, nreps, nvect,nvesc,store(l,m)

      real*4 sp,sumx(maxl,maxm),sumy(maxl,maxm),denom(3),nel(10),
     1 rinfo(6,nvect),ug(maxl,maxm),vg(maxl,maxm),ermx(10),gamma(9),
     2 rainf(10), work(maxl,maxm),time(nvect),
     3 a(3,maxl,maxm),udev(nvect),vdev(nvect),alat(nvect), 
     4 trainf
...
c
      ...
      d=sqrt(float(l1*m1)/float(nvect))
c
      print 1010,d
...
d     type *,'Nreps = ',nreps      
 ...
 1990   format(' ************* pass number ',i3,' **************' )
        ...
        ermax=ermx(ipass)
       print 1991, r, ermax
 1991   format ('Rainf= ',f6.2,', ermax= ',f6.2)
c
...
cd          endif 
d          if (abs(udev(ist)).gt.ermax)badone=1
d          if (nvesc.eq.1.and.abs(vdev(ist)).gt.ermax)badone=1
d          if(ipass.eq.1.and.badi.lt.20.and.badone)then 
d            type *,'bad piece of data inside grid, vector # =',ist
d            type *, '  lon/lat = ',xlon,xlat
d            type *,'   u/v = ',rinfo(5,ist),rinfo(6,ist)
d            type *,'  u/v guess = ',uval,vval
d            type *,'  u/v dev = ',udev(ist),vdev(ist)
d            badi=badi+1
d          endif 
...
d        print *,'Number of eliminated points inside grid',nel(ipass)
...
d         write(22,168)ist,xlat,xlon,rinfo(5,ist),rinfo(6,ist),
d    1      uval,vval
          nel(ipass)=nel(ipass)+1
44        continue
          udev(ist)=999.
...
C       -- For each 'station' find the guess field locations 
C          within the current radius of influence,   
        first=1
        do 70 ist=1,nvect
          id=ifix(abs(rinfo(1,ist))+0.01)
...
cd             if (i.eq.183 .and. j.eq.114) then 
cd                type *,'(60) i,j,weight,a,store,sumx,sumy,x,y,xx,yy'
cd                write(22,203) i,j,weight,a(1,i,j),a(2,i,j),a(3,i,j)
cd    +          ,store(i,j),sumx(i,j),sumy(i,j),x,y,xx,yy
d203             format(2(i4,1x),1x,11(f8.3,1x))
cd             endif 
cd            if (i.eq.183 .and. j.eq.114) then 
cd              type *,'i .eq. 183, j .eq. 114'
cd              aa=a(1,i,j)
cd              bb=a(2,i,j)
cd              cc=a(3,i,j)
cd              if (first.eq.1) then 
cd                print *, 
cd    1 ' Lon,Lat,u,v,udev,vdev,a(1:3,i,j),w(s),sumx,sumy,num '
cd                 first=0
cd              endif 
cd              xlon=(x-1)*rlninc+rlnmn
cd              xlat=(y-1)*rltinc+rltmn
cd              dd=rinfo(5,ist)
cd              ee=rinfo(6,ist)
cd              print 170, xlon,xlat,dd,ee,
cd    1             udev(ist),vdev(ist),aa,bb, 
cd    2             cc,weight,sumx(i,j),sumy(i,j), 
cd    3             dist2,store(i,j)
cd170            format( f7.2, f7.2,',a(1.)= ', f8.2, ' ,a(2.)= ', f8.2,
cd    2          ',a(3.)= ',f8.2,',weight= ', f8.3,', sumx= ', f8.1, 
cd    3              ',sumy= ',f8.1, ',store= ', i4)
cd170            format( 2(f7.2,1x),7(f8.3,1x),f6.4,1x,f8.3,1x,
cd    1                  f10.3,1x,f8.3,1x,i4)
...
cd            endif 
cd            if (weight.ge.1.and.
cd    1              (xdist.gt.0.or.ydist.gt.0)) then 
cd                print 171, i,j,weight
cd171              format('i= ',i3,',j= ',i3,',weight= ',f6.4)
cd            endif 
...
d           if (i.eq.181 .and. j.eq.114) then 
d             print *,'i,j,ug,vg,a(1-3),p1dp2,xc,yc,sumx,sumy,store'
d             print 169,i,j,ug(i,j),vg(i,j),a(1,i,j),a(2,i,j),a(3,i,j),
d    1           p1dp2,xc,yc,sumx(i,j),sumy(i,j),store(i,j)
d169          format(5x,i3,1x,i3,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,
d    3          f8.3,f8.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.3)
d           endif 
...
d        print 200,k,urms,vrms,vec
 200   format(1x,'nbre de pts',i6,'rms u error=',f8.3,1x,
     ...
     2  'rms vector error=',f8.3,1x,'para unit')
      ...
      dummy=1
d      type*,"Checking for big numbers"
d      do ii=1,l
d        do jj=1,m
d          if (abs(ug(ii,jj)).gt.50.or.(abs(vg(ii,jj)).gt.50)) then 
d             type *,"|ug| or |vg| > 70"
d             write (22, 204) ii,jj,ug(ii,jj),vg(ii,jj)
d204  format('ii = ',i4, ',jj = ',i4,', ug = ',f8.3,',vg = ',f8.3)
d          end if
d        end do
d      end do
...
C     ===== Done with the big loop !=============
      ...
      if(nvesc.eq.1)then 
d        type*,' calling smooth for vg'
        call smooth(sp,vg,work,l,m)
      endif 
d      type*,' calling smooth for ug'
