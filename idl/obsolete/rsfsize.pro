PRO rsfsize, animpar, loncent, maxnland,numland
  maxnland = -1l
  set_plot,'z'
  ...
  frames_file = 'RSF-' + xs_str + ',' + ys_str + ',' + $
   nframes_str + ',' + loncent_str + '.hdf'

  rsf = obj_new('rsf',frames_file,/write, animpar, loncent)
  ...
  FOR frm=0,nframes-1 DO BEGIN 
    print,'working on frame', frm
     ...
     IF frm EQ 0 THEN rsf-> writeMappedPixels, mappedPixels
     ...
     land=where(tmask eq 1,nland)
      ...
      IF rsf-> WriteData( frm, landi, landeli) EQ 0 THEN BEGIN 
