...
;                          auto_movie2.pro_defs.dat, which currently
;                          resides in $VAP_LIB. 
;                
...
PRO auto_movie2.pro, date_time, $ ; (I) end time of data used in movie 
  ...
  auto_movie2_cronjob = 0
  IF n_Elements(pid) NE 0 THEN $
  auto_movie2_cronjob = ( CheckForLock( pid, 'auto_movie2.lock', $
                                     user, dir='/tmp') EQ 1)
  catch, error_status
  IF error_status NE 0 THEN BEGIN
    IF auto_movie2_cronjob THEN BEGIN 
      IF exist(llun) THEN $
  ...
  IF n_elements( roi ) NE 0 THEN BEGIN 
    roistr =  READ_AUTO_MOVIE2.PRO_DEFS( roi )
  ...
  ENDIF ELSE BEGIN 
    roi =  'NPAC'
    roistr =  READ_AUTO_MOVIE2.PRO_DEFS( 'NPAC' )
  ENDELSE 
