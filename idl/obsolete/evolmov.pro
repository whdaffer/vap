PRO incrementpath, u,v,ui,vi,loni,lati,loni0,lati0,pathinc,nframes,timei,$
  ...
  ENDIF ELSE BEGIN 
    ...
    loni = (loni + cos(lati)*ui*pathinc) MOD 360
...
; CATEGORY:  Animation
;
; CALLING SEQUENCE:  EvolMov, files, nframes, filetimes,
...
PRO evolmov, files, nframes, filetimes, object=object, $
...
;  ENDIF 
  ...
  IF NOT vetted THEN BEGIN 
    lf = string(10b)
    ustr =  lf + "Usage: evolmov, files, nframes ,object=object [, filetimes, " + lf 
    ustr =  ustr + $
  ...
  IF n_elements(lonpar)  EQ 0 THEN lonpar = [0.,360]
  IF n_elements(latpar)  EQ 0 THEN latpar = [-60,60]
  ...
  range = maxv-minv
  levels = findgen(30)/29*range+minv
  ...
  FOR frm=0,nframes-1 DO BEGIN 
    ...
    IF animobj-> type() EQ 'VECTOR' THEN BEGIN 
      u = curdat[*,*,0]
      v = curdat[*,*,1]
      ...
      CONTOUR,s,lon,lat,levels=levels,$
