...
COMMON colors, r_curr, g_curr, b_curr, r_orig, g_orig, b_orig
  ...
  '  png=png, $    ; flat to write PNG files' + lf + $
...
forward_FUNCTION cfgname

read_cfgfile = 0
IF n_elements(cfgname) EQ 0 THEN BEGIN 
  cfgname = cfgname()
  cfgpath = '~/.idlcfg/' 
...
IF n_elements(pict) NE 0 THEN pict = KEYWORD_SET( pict )  
IF n_elements(gif) NE 0 THEN gif     = KEYWORD_SET( gif )  
IF n_elements(ps) NE 0 THEN ps      = KEYWORD_SET( ps )   
IF n_elements(tiff) NE 0 THEN tiff    = KEYWORD_SET( tiff )  
IF n_elements(jpeg) NE 0 THEN jpeg    = KEYWORD_SET( jpeg )  
IF n_elements(png) NE 0 THEN png     = KEYWORD_SET(png)
...
CASE 1 OF
  ...
  png: Message,' Files will be output in PNG format',/cont
  ELSE : BEGIN
...
;      print,' Working on Frame # ', frm
      ...
      CASE 1 OF 
        ...
        png: BEGIN 
          tfile = 'nwind.' + frame
          write_png, tfile, outim,red,green,blue
        END 
        ELSE: BEGIN 
          Message,"Unrecognize output format!",/cont
          return
        END 
      ENDCASE 
