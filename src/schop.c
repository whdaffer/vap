 ...
 * schop.c
 *
 * Chop the sigma0s off the end of each SeaWinds on ADEOS-II MGDR
 * record and write the truncated result out to a file in the current
 * working directory whose name is SWYYYYDDMM.SHHM.EHHMM where the
 * SHHMM is the starting hour/minute and the EHHMM is the ending
 * hour/minute. This name is constructed from information contained in
 * the header of the MGDR file, specifically the DataStartTime and the
 * DataEndTime.
 *
 *
 * $Id$
...
void main( int argc,char **argv ) {
  ...
  if (argc<2) 
    {
      fprintf(stdout,"Usage: qchop [-n] SW_NRT_FILE\n");
      exit(1);
    }
