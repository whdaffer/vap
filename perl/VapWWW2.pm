...
#
#
# This package is *exactly* the same as VapWWW.pm. Additionally it has
# the CGI code to handle the requests for the Tropical storm archive
# page.
# 
# I keep meaning to transition to only using this package, but I
...
@EXPORT=qw( auto_movie_defs doy2mday_mon 
	   ...
	   ListofStorms choseRegion choseYear choseStorm 
	   generateStormPage pickem  generateStormListing getListing);
...
BEGIN {
  ...
  %region2descript=( GMS5  =>"West Pacific",
		     GOESEAST => "North Atlantic",
		     GOESWEST => "North East Pacific");
...
#      if (m{^.*                                           # stuff at beginning
# 		<a href=\"(.*/(\w+)-(\w+)-(\w+)-(\d+)\.jpeg)\"> # the filename
# 		    (.*)                                 # the HOT link
# 			\s*.*<img src=.*></a>.*          # some junk
# 			    \(\s*Processed:\s*(.*)\s*,\s*   # the process time
# 			      Size:\s*(\d+\s*[KkBb]*\s*)\).*$}x) { # and the size. 
...
sub getStormImages{
  ...
  ## $VAP_WWW_TOP/images/storms
  chdir "$VAP_WWW_TOP/images/storms" || 
      croak "Can't cd to $VAP_WW_TOP/images/storms\n";
  
  opendir DIR, "." || croak "Can't open images/storms directory!\n";
  @files=grep /.*\.jpeg$/, readdir(DIR);
  closedir DIR;
  @files=sort @files;
}
...
sub ListofStorms {
  my $thisregion = shift;
  my $thisyear = shift;
  local(@files)=getStormImages();
  local(%hash) = ();
  for (@files){
    local($region,$type,$storm,$time) = split "-";
    local($year,$rest) = $time =~ /(\d{4})(\d+)/;
    next unless $region eq $thisregion;
    next unless $year eq $thisyear;

    $name=join /\|/, ($region,$storm,$year);
    $hash{$name}++;
  }
  @storms=keys %hash;
}
...
sub choseRegion{
  ...
  print header, start_html('Chose the Region');
  print h1("Pick the region you want to view");
  print a({-href=>'/cgi-bin/pickem.pl?region=GMS5'},$region2descript{GMS5});
  print p("This region runs from Longitude 70 to 190 and Latitude +/- 60",
	  "and primarily covers the Western Pacific");
  print a({-href=>'/cgi-bin/pickem.pl?region=GOESWEST'},$region2descript{GOESWEST});
  print p("Covers Latitude 0 to 70 and Longitude 180 to 270,",
	  "roughly the North West coast of the U.S.");
  print a({-href=>'/cgi-bin/pickem.pl?region=GOESEAST'},$region2descript{GOESEAST});
  print p("The Northern Atlantic");
  print end_html;

}

sub choseYear{
  # the single argument should be a string like 
  #
  #  $_[0] = "GOESWEST|1999|2000|2001"
  #
  local (@years, $region, $descript, $y );
  @years=split("/",$_[0]);
  $region =shift @years;
  $descript=$region2descript{$region};

  print header, start_html("Chose the Year");
  print h1("Pick which year you wish to see for the $descript");
  for (@years){
    print p,a({-href=>"/cgi-bin/pickem.pl?region=$region&year=$_"},$_);
  }
  print end_html;
...
sub choseStorm{
  ...
  # The single argument should be a string of the form
  #
  # $_[0] = "GOESWEST|YYYY|name1|name2|name3...|nameN"
  #

  local (@names, $region, $descript, $year, $n );
  @names=split("/",$_[0]);
  $region =shift @names;
  $descript=$region2descript{$region};
  $year=shift @names;
  print header,   start_html("Chose the Storm");
  print h1("Chose the storm from the $descript, in year $year");
  for (@names){
    print a({-href=>"/cgi-bin/pickem.pl?region=$region&year=$year&name=$_"},"$_",p);
  }
  print end_html;
}
...
sub getYears{
  local($region)=shift;
  @files=getStormImages;
  local(%year,@years);
  for (@files){
    local($r,$t,$s,$tt) = split "-";
    local($y,$rest) = $tt =~ /(\d{4})(\d+)/;
...
sub pickem {
  ...
  if ($#params == -1) {
    ...
    $region=param('region');
    checkValue( $region );
    @years=getYears($region);
    if ($#years == 0) {
      @names=getNames($region,$years[0]);
      if ($#names == 0) {
	generateStormListing($region, $years[0], $names[0]);
      } else {
	$string = join "/", ($region, $years[0], @names);
	choseStorm $string;
      }
    } else {
      $string=join "/",($region,@years);
      choseYear $string;
    }
  } elsif ($#params == 1){
    $region=param('region');
    ...
    $year=param('year');
    checkValue( $year );
    @names=getNames( $region, $year );
    $string = join "/", ($region, $year, @names);
    choseStorm $string;
    ...
    $name=param('name');
    checkValue( $name );
    generateStormListing( $region, $year, $name);
  }
}
...
sub checkValue {
  ...
  $test=$_[0];
  unless ($test =~ m#^([\w-]+)$#) {
	  die "Value ($test) has disallowed characters\n";
  }
  $test=$1;
}
sub generateStormListing {
  local(@files)=getListing(@_);
  croak "No files!\n" if $#files<0;
  local($r,$t,$s,$tt) = split "-", $files[0];
  local($y,$rest) = $tt =~ /(\d{4})(\d+)/;
  local($descr)=$region2descript{$r};
  print header, start_html("Region: $descr, Year: $y, Storm: $s"), "\n";
  for (@files){
    local($r,$t,$s,$tt) = split "-";
    local($y,$m,$d,$h,$mm) = $tt =~ /(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})/;
    local($time) = join "/", ($y,$m,$d,$h,$mm);
    print p,a({-href=>"/images/storms/$_",},"$time",p,
	      img({-src=>"/images/storms/$_", 
...
sub getListing{
  ...
  for (@files){
    ...
    next unless $region eq $r && 
	$s eq $name &&
	    $y == $year;
    push @retfiles, $_;
  }
  @retfiles = sort bytimeinname @retfiles;  
}


sub topLevArchiveChoices{

  local(@stormlist)=ListofStorms();
  local(%region, %desig,@namelist);
  for (@stormlist){
    local($region,$name,$year) = split /\|/;
    $desig{"$region/$year"}++;
    push @{$region{$region}{$year}}, $name;
  }
  @desig=sort keys %desig;
  print header, start_html('Archive Listing of Storms');
  print start_form();
...
sub genListPage{
  local ($nbspc="&nbsp;");
  local(%regionnames) = ( GMS5 => "West Pacific", 
		     GOESEAST => "North Atlantic",
		     GOESWEST => "North East Pacific");

  local(@stormlist)=ListofStorms();
  local(%years, @namelist);
  for (@stormlist){
    local($region,$name,$year) = split /\|/;
    push @{$region{$region}{$year}}, $name;
  }
  ...
  print header, start_html('Archive Listing of Storms');
  print start_form(-action=>"/cgi-bin/listing-form.pl");
  
  @regions= sort( keys %region);
  foreach $r (@regions){
    print h1($regions{$r});
    @years = sort( keys %{$region{$r}} );
    foreach $y (@years){
      @namelist=@{$region{$y}};
      if ($#namelist >= 0) {
	print p('Storms in year $y');
	print p('Please select one of these storms');
	foreach $n (@namelist) {
	  $labels{$r-$n-$y} = $n;
	  push @values, $r-$n-$y;
	}
	print popup_menu( -name=>"$r_$y_STORMS",
		   -value=>@values, -labels=>%labels);
      }
    }
    print p();
    print p();
...
sub generateArchiveListingPage{
  local ($nbspc="&nbsp;");
  local(%regions) = ( GMS5 => "West Pacific", 
		     GOESEAST => "North Atlantic",
		     GOESWEST => "North East Pacific");
  
  $head=pageHead;
  @lines=($head);
  push @lines, "<FORM ACTION=\"/cgi-bin/stormsarchive.pl\" METHOD=POST>";
  ...
  @regions= sort( keys %region);
  foreach $r (@regions){
    push @lines, "<h1>$regions{$r}</h1>";
    @years = sort( keys %{$region{$r}} );
    foreach $y (@years){
      @namelist=@{$region{$y}};
      if ($#namelist >= 0) {
	push @lines, "Storms in year $y<p>";
	push @lines, "Please select one of these storms<p>";
	push @lines, "<SELECT NAME=\"$r_$y_STORMS\">";
	foreach $n (@namelist) {
	  push @lines, "<OPTION value=\"$r-$n-$y\"> $n";
	}
	push @lines, "</SELECT>";
      }
    }
    push @lines, ($nbspc,$nbspc);
  }
  push @lines, "</FORM>";
  @lines;
}
...
sub bytimeinname{
  local($sa,$ta,$na,$ta,@r) = split "-",$a;
  local($sb,$tb,$nb,$tb,@r) = split "-",$b;
  $ta <=> $tb;
}
1;
