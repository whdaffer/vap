...
sub new {
  ...
  my $ncols=3;
  my $nrows=5;
  my $self=$class->SUPER::new(-rows=>$nrows,-cols=>$ncols, 
			      -valign=>"top", @_);
  ...
  my $overlay_table=HTML::Table->new(-cols=>2,-rows=>2,
				     -align=>'CENTER');
  ...
  $overlay_table->setCellRowSpan(1,1,2);
  ...
  $overlay_table->setCell(2,2,$link);
  ...
  my $ts_table=HTML::Table->new(-cols=>2,-rows=>2,
				-align=>'center');
  ...
  $overlay_table->setCellVAlign(1,1,'CENTER');
  $overlay_table->setCellRowSpan(1,1,2);
  #$overlay_table->setCellNoWrap(1,1,1);
  ...
  my $anim_table=HTML::Table->new(-cols=>1,-rows=>1,
				  -align=>'center');
 ...
 $col=1;
  ...
  $self->setCellAlign($row, $col,"CENTER");
  $self->setCellColSpan($row++, $col,4);
